...
- **`/dev-harness`, the delivery loop run across separate terminal panes.** It drives [Herdr](https://herdr.dev) panes as a roster: a coordinator that relays, a developer that runs `/dev-loop` and the recovery skills, and a reviewer on a different model that runs `/dev-check review`, `/dev-audit`, and `/dev-qa`. `/dev-harness config` records the roster in `.konteksto/harness.md` and refuses a reviewer on the developer's model, which is what makes the different model review gate structural instead of hopeful. The coordinator dispatches only a route that `/dev-loop` or `/dev-audit` already recorded, and asks a person when nothing is recorded, so the routing rules keep exactly one home. Requires `HERDR_ENV=1`, and stops rather than falling back to anything weaker.
...
- **A relay that reaches the person wherever they are, with Claude Remote Control first.** A decision a person owns, a blocked approval dialog, a finished phase, a worker that ran out of quota, and a role that escalated all reach the person, and their answer is delivered to the pane that asked so the owning skill records it. When the coordinator runs Claude, `config` recommends Remote Control: the coordinator pushes a one line notice to the person's phone and they answer inside that session, so there is no bot to create, no token to keep out of a file, and no channel a third party can post into. It is recommended subject to a test, never assumed, because an organization can switch the feature off for an account and the flag is still accepted: the agent starts, reports itself idle, and reaches nobody. `config` sends one notice, treats the organization policy message or a session the person cannot see as a failure, and falls back to Telegram or the coordinator pane rather than recording a relay that is not there. Telegram is offered when the coordinator runs something else, in two transports: the shipped poller, which holds the update offset and keeps the token out of the command line, or plain curl, which installs nothing. Telegram inbound is untrusted, so only the configured chat and an allowlist of senders are accepted and only a fixed set of first words is a control instruction; anything else is weighed as an ordinary instruction under the same ownership rules. With no relay at all, every question is asked in the coordinator pane and the routing does not change.
