...
- **A design review session, driven by Playwright.** Before asking for approval, `/dev-architect` now renders the proposal at every breakpoint `design.md` defines and every state the registry says the surface has, exercises the interactions the flow names, and collects every console message, page error, and failed request it produced. It serves a working copy plus that evidence on a loopback address and hands a person the review page, which offers Approve, Request changes, and Reject. `internal/design-review.md` defines the session, and nothing else may redefine it.
