...
- **No review report:** if `.konteksto/reviews/` holds no report covering the selected range, the audit has not run. Do not read an empty `audit-register.md` as a clean result: both files being empty is one fact stated twice, and it is that nobody has looked. If this session may run `/dev-audit`, run it and wait for the report it produces. If it may not, because another window owns that step, set the phase to `blocked`, record that the audit phase was reached with no report produced and which window owes it, and stop. **Do not hand back with the phase still `audit` and nothing changed.** That is a phase value that cannot tell audit owed from audit done, and across two windows it becomes a cycle where each side correctly refuses and neither reports it.
