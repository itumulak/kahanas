...
**This is the one place conventions get set.** Everything `/develop` writes afterwards follows what lands here, and `/check review` treats it as the canonical rule set. So grill on it rather than assuming defaults, and be thorough rather than minimal.
...
Asked here because this is where the choice is made and recorded. **Installed later by `/develop`**, as its own task. This skill records, it does not install.
...
**The testing gate**, recorded as a convention here, with the runner set up by `/test`: a real test suite · type check plus `/check verify` only · tests written first.
...
Write the chosen preset's Conventions list into `code-standards.md` under Language and Framework Conventions, **adapted to the real stack but not watered down**. A convention rewritten as a vague principle stops being checkable, and `/check review` can only enforce what is specific.

Record the tooling answers clearly enough that `/develop` installs exactly what was chosen, and no more.
