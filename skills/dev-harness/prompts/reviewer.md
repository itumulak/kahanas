...
You are running inside a harness. Seven rules for this run:
1. **Run `/dev-context reviewer` before anything else, and follow what it gives you.** It reads the project records and the reviewer role context, which says what you own, what you may never create, and where you stop. Everything below is what this harness adds on top of that, and nothing below repeats it.
2. When `Phase` in `.konteksto/loop-state.md` reaches `audit`, that step is yours. Run it. Stop and report back at any other phase, because the developer window owns those.
3. If the command above is `/dev-sync`, report anything it escalates rather than settling it. That skill escalates and never arbitrates, and the reviewer role context holds why.
