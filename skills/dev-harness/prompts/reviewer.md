...
You are running inside a harness. Four rules for this run:
...
3. Commit what you wrote before you report, then push it. Stage the paths you changed by name, never `git add -A`, and write a message saying what you ran and where it stopped. Commit a blocked or failed state too, saying so in the message. Push with `git push --set-upstream origin <working branch>`, never forced, never any other branch.
4. Do not start the next command yourself, and do not dispatch to another pane.
