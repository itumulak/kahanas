...
description: "Run /check to confirm a task is sound before moving on. Two modes: verify runs the real app and proves the behavior against the flows in project-overview.md; review runs a senior code read on a different model than wrote the code. Verify after /develop, review before a merge. Never edits your code."
---
...
**Neither mode edits code.** Verify points failures at `/develop`. Review reports findings for someone else to fix. A checker that also fixes stops being a checker.

## Where this sits

**Before this:** `/develop`, for verify. Any finished change, for review.
...
**Never mark a task done.** That is `/develop`'s line, and only after this passes.
...
Route before touching the repo. Look at what followed `/check`:
...
- **Anything else**, including a bare `/check` or a task name with no mode, do not guess and do not default. Ask, and wait:

> Which check do you want?
> **verify**, run the real app and prove the task works against its flow, usually right after `/develop`.
