...
The `verify` mode of `/check`: run the real app and prove the task works. Follow it fully.
...
Never modifies application code. Report the breakage and point at `/develop`.
...
**Failed**:
- <behavior>: <what went wrong, with the exact error or screenshot path> → run /develop
...
This mode confirms reality, and never fixes or asserts. `/develop` fixes a failure and builds a surface that is missing or not live.
