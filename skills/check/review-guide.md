...
`TESTS = none-by-design`, the project deliberately has no runner and gates on the type checker plus `/check verify`:
...
- Weigh correctness and security findings on their own merit instead. Where a change is risky, point at what the gate should catch, for example that the type checker will not catch a runtime shape and `/check verify` should exercise it.
