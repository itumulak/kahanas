...
description: "Run /debug to find and fix the root cause of a bug: something failing, throwing, or behaving wrong, when /check verify reports a failure or behavior is unexpected. Runs a reproduce, localize, hypothesize, test, fix, verify loop, makes the smallest fix, and records what it found. No features, no extra refactors."
---
...
**Writes** the minimal code fix for the proven root cause, and one line in `progress-tracker.md` under Decisions Made During Build recording the cause and the fix. That log is where a later session finds out this ground was already covered.

**Never writes** a feature, a refactor of unrelated code, or any of the design documents. If the bug turns out to be a flawed decision rather than a coding mistake, say so and point at `/architect`. Papering over a wrong design with a code patch buys one day and costs many.
...
Arriving from `/check verify`? Its report already holds all three, since a verify failure quotes the decisive line and names what it ran. Start from that rather than asking again.
...
**A fix that requires changing a design document is not a fix you make here.** Stop and route to `/architect`.
...
- **Add a regression test** that fails without the fix and passes with it. Write it inline when that is fastest, or hand the case to `/test`.
...
**Regression test**: <added inline | case handed to /test>
**Siblings**: <the same cause found elsewhere, fixed or reported | none found>
**Recorded**: <the line added to progress-tracker.md>
**Deeper issue**: <a design document this proves wrong, run /architect | none>
```
