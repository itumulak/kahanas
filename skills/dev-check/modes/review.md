# /dev-check review (fresh model code review)

The `review` mode of `/dev-check`: a senior code review, before merge, on a different model than wrote the code. Follow it fully.
...
Read `test-preferences.json` at the project root, which `/dev-test` owns and maintains:
...
This mode is complete after relaying. It does not fix the findings, and it does not invoke another skill. Fixing them is `/dev-develop`'s job, and a normal follow up.

Once the findings are dealt with, the remaining steps before a merge are `/dev-document pr` to write the description, and `/dev-sync` to make the documents true again. Name both, since neither is obvious from here.
