...
**Verify** owns no whole file. Chat output only, plus screenshots and logs in a scratch area, and two narrow writes. It stamps the task's **Verify Check** cell in `progress-tracker.md`, `PASSED` or `FAILED`, and it appends an Evidence row to `decision-log.md` on a pass, which is how a later session tells an exercised task from an assumed one. That one column is all of `progress-tracker.md` it may touch: Status belongs to `/dev-develop`, and checkpoint rows and assignees to neither of them.
