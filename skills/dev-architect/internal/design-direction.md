...
**On a codebase that was already shipped, step 2a settled which of those rows are `MISSING` and which are `BASELINE`.** Apply that answer here, and write its one line note above the Entries table. The mapping itself still runs in full: **the baseline changes what a row says, never whether the row exists**, and a registry missing its existing surfaces cannot tell a shipped screen from one nobody noticed was needed. `internal/adoption-baseline.md` has the questions, and the registry's Status values section defines what `BASELINE` means and what puts a surface back into the lifecycle.
