...
**That preferences file is the test signal the rest of the workflow reads.** `/check review` resolves it to decide whether missing coverage is a finding at all, so getting it right matters beyond this skill:
...
**Write both `tool` and `gate` every time, never just one.** `/test` reads `tool` to decide whether to write a suite at all, and `/check review` reads `gate` to tell a project that deliberately chose no runner apart from one that simply never set tests up. **A file carrying neither key is malformed**, and it makes both skills guess.
...
- **Already stated**, in `code-standards.md` or by the user, that this project gates on the type checker plus `/check verify`: respect it. Save the gate shape, run the type check as the gate, and point at `/check verify` for behavior. **Do not push a framework on a project that decided against one.**
- **Not stated**: ask, and do not default to installing.

The three real options:

1. **Set up a framework**: continue to the question below.
2. **No runner, gate on the type checker plus `/check verify`**: save `"tool": null` with the gate, run the type check, and defer behavior to `/check verify`. Never install anything.
...
- **The code is wrong**: that is a real bug, so stop and route to `/debug`. Do not quietly reshape the test until it passes, which is the single easiest way to make a suite worthless.
