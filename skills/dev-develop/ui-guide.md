...
1. **`design.md`**, the art direction. Character, the build mandate, composition patterns, component rules, the states, and a pointer to where the real tokens live. **This is the source of truth for how the product looks**, and `/dev-architect` settled it before any of this was built.
...
**No `design.md` at all?** That means either a backend only project, in which case you should not be on this track, or that `/dev-architect` has not run yet. Stop and say so, rather than inventing a direction that the next session will contradict.
