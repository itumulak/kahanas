...
On **team**, `/dev-architect` adds an Assigned column to every phase table in `progress-tracker.md` and an actor column to `decision-log.md`, so the plan records who owns a task and the log records who ran each event. On **personal**, both are left out, since there is only ever one answer.
